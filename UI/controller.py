...
class Controller:
    ...
    def handleCercaRaggiungibili(self,e):
        if self._fermataPartenza is None:
            self._view.lst_result.controls.clear()
            self._view.lst_result.controls.append(ft.Text("Non è stata fatta una scelta di stazione di partenza", color="red"))
            self._view.update_page()
            return
        ...
        for n in nodes:
            self._view.lst_result.controls.append(ft.Text(f"{n.nome}"))
        self._view.update_page()
